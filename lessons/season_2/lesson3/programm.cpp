...
void who_this_min (int &mass_size, int *&vec, int *&min_vec, int **&tabl, int &min_sum);	// Find min summ and make min_vec on iteration.
...
int main( )
{
	...
	while (true)
	{
		who_this_min (mass_size,vec,min_vec,tabl,min_sum);
...
void who_this_min (int &mass_size, int *&vec, int *&min_vec, int **&tabl, int &min_sum)
{
	if (assign(vec,tabl,mass_size)<min_sum)
	{
		min_sum=assign(vec,tabl,mass_size);
		for (int a=0; a<mass_size;min_vec[a]=(vec[a]+1), a++);
	}
	return;
}
